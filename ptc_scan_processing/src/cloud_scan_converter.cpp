...
CloudScanConverter::CloudScanConverter(ros::NodeHandle &nh, std::unordered_map<std::string, float> &params, std::unordered_map<std::string, bool> &flags,
...
const bool CloudScanConverter::filterRoverPoints(const Eigen::Vector4f &p)
{
...
const bool CloudScanConverter::filterRange(const Eigen::Vector4f &p)
{
    return p.head<2>().norm() > max_xy_range_;
}

const bool CloudScanConverter::filterIntensity(const float intensity)
{
