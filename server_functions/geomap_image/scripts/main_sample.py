...
if __name__ == "__main__":
    ...
    robot_map_manager = MapManager(rover_code=rover_code, folder=map_folder, imsize=desired_map_size)
    # Call the point cloud and bev generations
    map_ptc = robot_map_manager.generateMapPtc()
    map_bev, map_coords_json = robot_map_manager.generateMapBev(ptc_map_frame=map_ptc)
    # Save the files
    robot_map_manager.saveMap(map_bev=map_bev, map_ptc=map_ptc, map_coords=map_coords_json)
    end = time()
